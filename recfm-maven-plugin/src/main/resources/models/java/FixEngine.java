...
abstract class FixEngine {
    ...
    protected void setAbc(String s, int offset, int count, OverflowAction overflowAction, UnderflowAction underflowAction, char pad) {
        if (s==null) {
            ...
            switch (underflowAction) {
                case PadRight: padAbcToRight(s, offset, count, pad); break;
                case PadLeft: padAbcToLeft(s, offset, count, pad); break;
                case Error: throw new FixError.FieldUnderFlowException(FIELD_AT+offset+EXPECTED+count+CHARS_FOUND+s.length());
            }
        } else switch (overflowAction) {
            case TruncRight: truncAbcRight(s, offset, count); break;
            case TruncLeft: truncAbcLeft(s, offset, count); break;
            case Error: throw new FixError.FieldOverFlowException(FIELD_AT+offset+EXPECTED+count+CHARS_FOUND+s.length());
        }
    }
    ...
    protected void setNum(String s, int offset, int count, OverflowAction ovfl, UnderflowAction unfl, char fill) {
        if (s==null) {
            ...
            switch (unfl) {
                case PadRight: padNumToRight(s, offset, count); break;
                case PadLeft: padNumToLeft(s, offset, count); break;
                case Error: throw new FixError.FieldUnderFlowException(FIELD_AT+offset+EXPECTED+count+CHARS_FOUND+s.length());
            }
        } else switch (ovfl) {
            case TruncRight: truncNumRight(s, offset, count); break;
            case TruncLeft: truncNumLeft(s, offset, count); break;
            case Error: throw new FixError.FieldOverFlowException(FIELD_AT+offset+EXPECTED+count+CHARS_FOUND+s.length());
        }
    }
