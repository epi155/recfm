...
import io.java.sys.FixError;
import io.java.sys.ValidateError;
...
class TestFields {
    ...
    void testDom() {
        ...
        foo.validate((String name, int offset, int length, int column, ValidateError code) -> {
            System.out.printf("Error on field %s at offset %d, length %d, code %s%n",
                name, offset, length, code.name());
            System.out.printf("Value: /%s/%n",
                foo.encode().substring(offset, offset+length));
        });
    }
    @Test
    void testGrp() {
